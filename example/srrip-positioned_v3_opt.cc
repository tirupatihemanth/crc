...
uint32_t CONSTITUENCY_SIZE=3; //For every 3 sets we store the Signatures in one set
...
void removeDeadSignatures(){
  ...
  cout<<"SIZE OF ATD WITH DEAD SIGS: "<<ATD.size()<<endl;
  map<SIG, ACCESS_STATS*>::iterator it = ATD.begin();
  while(it!= ATD.end()){
    cout<<"IN WHILE "<<it->first<<endl;
    if(it->second == NULL){
      cout<<"CAUGHT IT"<<endl;
      //assert(false);
      it = ATD.erase(it);
...
void positionedInsertion(SIG sign, uint32_t set, uint32_t way){
    // POSITIONED INSERTION POLICY
    // On each fill we insert based on the INS_POS calculated from the previous EPOCH for this sig
    if(set%CONSTITUENCY_SIZE!=0 && ATD[sign]==NULL){
      rrpv[set][way] = MAX_RRPV-1;
      return;
    }

    assert(ATD[sign]->INS_POS <= MAX_INS_POS);
    if(ATD[sign]->INS_POS==MAX_INS_POS){
      rrpv[set][way] = MAX_INS_POS-1;
    }
    else{
      rrpv[set][way] = ATD[sign]->INS_POS;
    }
}
