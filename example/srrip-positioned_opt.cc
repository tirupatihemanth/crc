...
//typedef uint16_t SIG; //TODO: Appropriately typedef SIG to sufficient size ater getSignature implementation
...
uint32_t NUM_DEDICATED_SETS=2048; //TODO: Also check for 32
uint32_t CONSTITUENCY_SIZE=LLC_SETS/NUM_DEDICATED_SETS;
...
//Storing signatures only for the sampled sets
map <uint32_t, vector<SIG> > sig;
...
void stepHitPromotion(uint32_t set, uint32_t way){
  ...
  if(set%CONSTITUENCY_SIZE==0 && ATD[sig[set][way]]==NULL){
  ...
  if(set%CONSTITUENCY_SIZE==0 && ABANDON_RAND_BYPASS==1 && ATD[sig[set][way]]->INS_POS == MAX_INS_POS){
  ...
  if(RAND_PROMOTION==1 && rand_r(&PROMOTION_SEED)%PROMOTION_PROB==0){
    //sig may not be present for the non dedicated sets
...
void positionedInsertion(SIG sig, uint32_t set, uint32_t way){
    ...
    //We have obtained an insertion for a block not in our dedicated sets
    //As we donot store signatures in the cache lines of these sets we can never
...
void InitReplacementState()
{
    ...
    cout << "CONSTITUENCY_SIZE: "<<CONSTITUENCY_SIZE<<endl;
    ...
    for(int i=0;i<LLC_SETS;i+=CONSTITUENCY_SIZE){
      sig[i] = vector<SIG>(LLC_WAYS, 0);
    }
}
...
void UpdateReplacementState (uint32_t cpu, uint32_t set, uint32_t way, uint64_t paddr, uint64_t PC, uint64_t victim_addr, uint32_t type, uint8_t hit)
{
    ...
    if (hit){
      ...
      //Analyzing signature data only from 64 sets
      //We Analyze 1 set from every 32 sets
